...
impl<T: Sized> TreeBitmap<T> {
    ...
    /// Internal helper for longest match lookup.
    fn longest_match_internal(&self, nibbles: &[u8]) -> Option<(u32, AllocatorHandle, u32)> {
        ...
        best_match.map(|(result_hdl, result_index)| (bits_matched, result_hdl, result_index))
    }
    ...
    pub fn longest_match(&self, nibbles: &[u8]) -> Option<(u32, &T)> {
        self.longest_match_internal(nibbles)
            .map(|(bits_matched, result_hdl, result_index)| {
    ...
    pub fn longest_match_mut(&mut self, nibbles: &[u8]) -> Option<(u32, &mut T)> {
        match self.longest_match_internal(nibbles) {
            Some((bits_matched, result_hdl, result_index)) => Some((
                bits_matched,
    ...
    /// Internal helper for matches lookup.
    fn matches_internal(&self, nibbles: &[u8]) -> Vec<(u32, AllocatorHandle, u32)> {
        let mut cur_hdl = self.root_handle();
        let mut cur_index = 0;
        let mut bits_searched = 0;
        let mut matches: Vec<(u32, AllocatorHandle, u32)> = Vec::new();
    ...
    /// Return all matching prefixes for ```nibbles```.
    pub fn matches(&self, nibbles: &[u8]) -> Matches<'_, T> {
        Matches {
            inner: self.matches_internal(nibbles).into_iter(),
            results: &self.results,
        }
    }

    /// Return all matching prefixes for ```nibbles``` with mutable references.
    ...
    /// Internal helper for exact match lookup.
            ...
            if reached_final_node {
                match cur_node.match_internal(bitmap) {
                    MatchResult::Match(result_hdl, result_index, _) => {
                        return Some((result_hdl, result_index));
                    }
                    _ => return None,
                }
            }
    ...
    pub fn exact_match_mut(&mut self, nibbles: &[u8], masklen: u32) -> Option<&mut T> {
        match self.exact_match_internal(nibbles, masklen) {
...
pub struct Matches<'a, T: 'a> {
    inner: std::vec::IntoIter<(u32, AllocatorHandle, u32)>,
    results: &'a Allocator<T>,
}
...
/// Iterator for mutable matches.
pub struct MatchesMut<'a, T: 'a> {
    inner: std::vec::IntoIter<(u32, AllocatorHandle, u32)>,
    results: &'a mut Allocator<T>,
}
...
impl<'a, T: 'a> Iterator for MatchesMut<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.inner
            .next()
            .map(|(bits_matched, result_hdl, result_index)| {
                let ptr: *mut T = self.results.get_mut(&result_hdl, result_index);
                (bits_matched, unsafe { &mut *ptr })
            })
    }
}
